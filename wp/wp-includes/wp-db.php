...
class wpdb {
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'잘못된 데이터베이스 접두어'/*/WP_I18N_DB_BAD_PREFIX*/);
...
<p>If you don\'t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="http://wordpress.org/support/">WordPress Support Forums</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, htmlspecialchars( $db, ENT_QUOTES ), htmlspecialchars( $this->dbuser, ENT_QUOTES ) ), 'db_select_fail' );
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'%3$s이(가) 만든 워드프레스 데이터베이스 오류 %1$s(쿼리 %2$s)'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'워드프레스 데이터베이스 오류 %1$s(쿼리 : %2$s)'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
...
<h1>데이터베이스에 연결하는데 오류가 생겼습니다.</h1>
<p>이는 <code>wp-config.php</code> 파일에 사용자이름과 암호가 정확하지 않거나 <code>%s</code>의 데이터베이스에 접근할 수 없는 경우 입니다.  이는 호스트의 데이터베이스 서버가 동작하지 않는 것 일 수 있습니다.</p>
<ul>
	<li>사용자이름과 암호가 올바릅니까?</li>
	<li>올바른 호스트네임을 입력하였습니까?</li>
	<li>데이터베이스 서버가 실행중입니까?</li>
</ul>
<p>확실하지 않다면 관리자에게 이를 문의하시십시오. 더 도움이 필요하다면 항상 <a href=\'http://wordpress.org/support/\'>워드프레스 지원 포럼</a>에 오십시오.</p>
'/*/WP_I18N_DB_CONN_ERROR*/, htmlspecialchars( $this->dbhost, ENT_QUOTES ) ), 'db_connect_fail' );
	...
	function get_row( $query = null, $output = OBJECT, $y = 0 ) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- 출력형식은 OBJECT, ARRAY_A, ARRAY_N 중의 하나이어야 합니다.'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
