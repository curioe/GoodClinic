...
function wpmu_current_site() {
	...
	// Still no dice.
	if ( 1 == count( $sites ) )
		wp_die( sprintf( /*WP_I18N_BLOG_DOESNT_EXIST*/'그 사이트는 존재하지 않습니다. <a href="%s">%s</a>을(를) 시도해보세요.'/*/WP_I18N_BLOG_DOESNT_EXIST*/, $sites[0]->domain . $sites[0]->path ) );
	else
		wp_die( /*WP_I18N_NO_SITE_DEFINED*/'이 호스트에 정의된 사이트가 없습니다. 만약 이 사이트의 주인이라면, 도움을 위해 <a href="http://codex.wordpress.org/Debugging_a_WordPress_Network">워드프레스 네트워크 디버깅</a>을 확인하세요.'/*/WP_I18N_NO_SITE_DEFINED*/ );
}
...
function ms_not_installed() {
	...
	$title = /*WP_I18N_FATAL_ERROR*/'데이터베이스 연결 구축 에러 '/*/WP_I18N_FATAL_ERROR*/;
	$msg  = '<h1>' . $title . '</h1>';
	if ( ! is_admin() )
		die( $msg );
	$msg .= '<p>' . /*WP_I18N_CONTACT_OWNER*/'만약 당신의 사이트가 표시되지 않는다면, 이 네트워크의 소유자에게 연락하세요.'/*/WP_I18N_CONTACT_OWNER*/ . '';
	$msg .= ' ' . /*WP_I18N_CHECK_MYSQL*/'만약 이 네트워크의 소유자라면 MySQL이 정상적으로 실행되고 있는지 모든 테이블이 에러가 없는지 확인하세요.'/*/WP_I18N_CHECK_MYSQL*/ . '</p>';
	if ( false && !$wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) )
		$msg .= '<p>' . sprintf( /*WP_I18N_TABLES_MISSING_LONG*/'<strong>데이터베이스 테이블이 없습니다.</strong> 이 말은 MySQL 이 실행되고 있지 않으며, 워드프레스가 정상적으로 설치되지 않았거나,누군가가 <code>%s</code>을(를) 삭제했다는 뜻입니다. 당신은 지금 데이터베이스를 확인해봐야 합니다.'/*/WP_I18N_TABLES_MISSING_LONG*/, $wpdb->site ) . '</p>';
	else
		$msg .= '<p>' . sprintf( /*WP_I18N_NO_SITE_FOUND*/'<strong><code>%1$s</code>의 사이트를 찾을 수 없습니다.</strong> <code>%3$s</code> 데이터베이스 안의 <code>%2$s</code>테이블을 검색했습니다. 맞습니까?'/*/WP_I18N_NO_SITE_FOUND*/, rtrim( $domain . $path, '/' ), $wpdb->blogs, DB_NAME ) . '</p>';
	$msg .= '<p><strong>' . /*WP_I18N_WHAT_DO_I_DO*/'What do I do now?'/*/WP_I18N_WHAT_DO_I_DO*/ . '</strong> ';
	$msg .= /*WP_I18N_RTFM*/'<a target="_blank" href="http://codex.wordpress.org/Debugging_a_WordPress_Network">버그 리포트</a> 페이지 읽기. 무엇이 잘못되었는지 도움이 될 수 있습니다.'/*/WP_I18N_RTFM*/;
	$msg .= ' ' . /*WP_I18N_STUCK*/'만약 아직도 이 메세지가 보인다면, 다음 테이블을 포함한 당신의 데이터베이스를 확인하세요:'/*/WP_I18N_STUCK*/ . '</p><ul>';
